...
def init_engines():
    ...
    # 【v2.6.6】场景/意图/情绪识别已移至LLM评分时，不再使用预分析
    # 保留classifier初始化但标记为弃用，避免兼容性问题
    cfg.classifier = None
    print("   📝 场景/意图/情绪识别 → 由LLM评分时统一识别（qwen2.5已弃用）")
...
def process_task_sync(task: dict, window_minutes: int = cfg.MERGE_WINDOW_MINUTES) -> bool:
    ...
    try:
        ...
        # 【v2.6.6】场景/意图/情绪识别已移至LLM评分时，不再预分析
        # 保留代码结构但跳过实际分类（如果classifier未初始化）
        if cfg.classifier:
        ...
        # 从评分结果中提取场景/意图/情绪（LLM识别结果）
        if result and 'session_analysis' in result:
            analysis = result['session_analysis']
            intent = type('Intent', (), {
                'scene': analysis.get('scene_category', '其他'),
                'intent': analysis.get('user_intent', '其他'),
                'sentiment': analysis.get('user_sentiment', 'neutral'),
                'confidence': 0.95,
                'sub_scene': analysis.get('scene_category', '其他'),
                'is_complaint': analysis.get('user_sentiment') == 'complaint',
                'complaint_type': '',
                'reasoning': 'LLM评分时识别',
                'source': 'llm_scoring',
                'latency_ms': 0.0
...
def run_grouped_parallel_worker(max_groups: int = 4, max_batch_size: int = 150,
    ...
    try:
        ...
        # 【v2.6.6】关闭分类器（如果存在）
        if hasattr(cfg, 'classifier') and cfg.classifier:
            try:
    ...
    try:
        while cfg.running:
            ...
            for task in all_tasks:
                ...
                # 【v2.6.6】场景/意图/情绪识别全部移到LLM评分时
                # 此处不再预分析,仅保留scene字段用于日志统计
                scene = task.get('scene', '未分类')
                task['_scene'] = scene
            ...
            print(f"\n📊 场景分布(历史数据): {scene_summary}")

            # 【v2.6.6核心优化】跨场景统一批量处理,场景/意图/情绪由LLM评分时识别
            print(f"\n🚀 跨场景合并处理: {len(all_tasks)} 通会话")
            print(f"   📝 场景/意图/情绪识别 → 移至LLM评分时")
